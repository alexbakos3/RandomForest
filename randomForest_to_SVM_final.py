...
review_body_vectorizer = TfidfVectorizer (max_features=4000, min_df=3, max_df=0.8, stop_words='english')
review_summary_vectorizer = TfidfVectorizer (max_features=4000, min_df=3, max_df=0.8, stop_words='english')
...
processed_summaries_RF_train = review_summary_vectorizer.transform(X_randomForestTrain['Summaries'].apply(lambda x: np.str_(x)))
...
processed_summaries_SVM_train = review_summary_vectorizer.transform(X_SVM_train['Summaries'].apply(lambda x: np.str_(x)))
processed_summaries_SVM_train = processed_summaries_SVM_train.toarray()

processed_summaries_cv = review_summary_vectorizer.transform(X_cross_validation['Summaries'].apply(lambda x: np.str_(x)))
...
summary_model = RandomForestRegressor(n_estimators=100, random_state=3)
...
body_model = RandomForestRegressor(n_estimators=100, random_state=3)
